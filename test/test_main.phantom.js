...
wru.test([
    {
        ...
        test: function () {
            ...
            wru.assert("minus", minus>400);
            wru.assert("plus", plus>400);
        }
    },
    {
      name:"Test random limit",
      test: function() {
        a = getRandomWithLimit(6,7);
        wru.assert("6-7", [6,7].indexOf(a) != -1);
        a = getRandomWithLimit(0,1000);
        wru.assert("<1000", a < 1000);
        wru.assert(">1000", a > 0);

        limit1 = 1000 * App.borderLimit;
        limit2 = 1000 - (1000 * App.borderLimit);
        wru.assert("on the limit", (a < limit1) || (a > limit2));
    ...
    },
    {
      ...
      test: function() {
        for (var k = 0; k<10; k++) {
          b = new Ball();
          b.init();
          wru.assert([-1,1].indexOf(b.dx) != -1);
          wru.assert([-1,1].indexOf(b.dy) != -1);

          wru.assert("random x", b.x < App.availW);
          wru.assert("random y", b.y < App.availH);
        }
      }
    },
    {
      ...
      test: function() {
        ...
        b2.y = 0;
        b2.radius = 40;
        wru.assert("collision",checkPositions(b1,b2)); 
        ...
        b2.y = 100;
        b2.radius = 40;
        wru.assert("not collision",!checkPositions(b1,b2)); 
      }
    },
    {
      name: "zeropad",
      test: function() {
        wru.assert(zeropad(89,7)=="0000089");
        wru.assert(zeropad("ABC",9)=="000000ABC");
      }
    }
]);
